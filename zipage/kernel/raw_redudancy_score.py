...
def raw_redudancy_score_kernel(
    ...
    if not n_block_id == -1:
        ...
        for n_block_offset in range(0, block_size, BLOCK_N):
            ...
            n_key = n_key / (n_norm)
            zo = tl.zeros((1, BLOCK_N), dtype=tl.int1)
            for m_block_idx in range(max_num_blocks_per_seq - 1, -1, -1):
                ...
                if not m_block_id == -1:
                    ...
                    for m_block_offset in range(block_size - BLOCK_M, -1, -BLOCK_M):
                        ...
                        zo = (last_threshold_mask).to(tl.int1) | zo
                        ...
                        s = tl.load(s_ptr, boundary_check=(0,))
                        s = similarity + s
...
def raw_redudancy_score(
    ...
    mask = (block_table == -1).unsqueeze(1).unsqueeze(-1).unsqueeze(0)
    redudancy.masked_fill_(mask, float("-inf"))
