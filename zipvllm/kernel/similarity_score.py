...
def norm_kernel(
    ...
    if block_id >= 0:
        seq_len = tl.load(seq_length_ptr + batch_idx)
        block_len = min(seq_len - block_idx * block_size, block_size)
        for block_offset in range(0, block_len, BLOCK_S):
...
def similarity_score_kernel(
    ...
    if m_block_id >= 0:
        seq_len = tl.load(seq_length_ptr + batch_idx)
        m_block_len = min(seq_len - m_block_idx * block_size, block_size)
        for m_block_offset in range(0, m_block_len, BLOCK_M):
            ...
            for n_block_idx in range(max_num_blocks_per_seq - 1, -1, -1):
                ...
                if n_block_id >= 0:
                    n_block_len = min(seq_len - n_block_idx * block_size, block_size)
                    for n_block_offset in range(0, n_block_len, BLOCK_N):
...
def cal_similarity(
    ...
    similarity_cos = similarity_cos.div_(seq_length.unsqueeze(-1).unsqueeze(-1))
